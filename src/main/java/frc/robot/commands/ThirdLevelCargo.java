...
public class ThirdLevelCargo extends CommandGroup 
{
  /**
   * Cargo to Third Level
   */
  public ThirdLevelCargo() 
  {
    addSequential(new ElbowToNinety()); //sets the elbow to full upright using limit switch
    addSequential(new LiftToHeight(80)); //lift to 4 inches below port hole
    addParallel(new StallLift(), 5); //stalls lift for 5 seconds
    addSequential(new ElbowToAngle(45)); //changes Angle
    addSequential(new Out(), 0.4); //pushes cargo out
